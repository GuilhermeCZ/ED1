...
tp_lista *SelectionSort(tp_lista *l){
  tp_nodo *x, *z, *aux, *menor;


  for(x = l->primeiro; x!=l->ultimo; x = x->proximo){
    menor = x;
    for(z = x->proximo; z!=NULL; z=z->proximo){
      if(z->valor < x->valor){
        menor = z;
      }
    }
    aux = x;
    if(menor != x){
      if(x->anterior != NULL && menor->proximo !=NULL){ //CASO OS DOIS SE ENCONTREM NO MEIO DA LISTA
        //------MENOR APONTANDO PARA VIZINHOS DE X--------
        menor->proximo->anterior->proximo = x->proximo;
        menor->anterior->proximo->anterior = x->anterior;
        //------
        //------VIZINHOS DE X ORIGINAL APONTANDO PARA MENOR
        x->proximo->anterior = menor->anterior->proximo;
        x->anterior->proximo = menor->anterior->proximo;
        //------
        //------VIZINHOS DE MENOR APONTANDO PARA X
        menor->proximo->anterior = x;
        menor->anterior->proximo = x;
        //------
        //------X APONTANDO PARA VIZINHOS DE MENOR
        x->proximo = menor->proximo;
        x->anterior = menor->anterior;
        //------
      }
      else if(x->anterior == NULL){ //CASO O X SEJA O PRIMIRO DA LISTA
        //------MENOR APONTANDO PARA VIZINHOS DE X--------
        menor->proximo->anterior->proximo = x->proximo;
        menor->anterior->proximo->anterior = NULL; //OU  "x->anterior" no lugar de NULL
        //------
        //------VIZINHOS DE X ORIGINAL APONTANDO PARA MENOR
        x->proximo->anterior = menor->anterior->proximo;
        //x->anterior->proximo   NÃO EXISTE POIS X->ATERIOR É NULL E NAO APONTA PARA NINGUEM
        //------
        //------VIZINHOS DE MENOR APONTANDO PARA X
        menor->proximo->anterior = x;
        menor->anterior->proximo = x;
      ...
      else if(menor->proximo == NULL){ //CASO O MENOR SEJA O ULTIMO DA LISTA
        //------MENOR APONTANDO PARA VIZINHOS DE X--------
        //menor->proximo->anterior->proximo = x->proximo;
        menor->anterior->proximo->anterior = x->anterior;
        //------
        ...
        //------
        //------X APONTANDO PARA VIZINHOS DE MENOR
        x->proximo = NULL; //OU "menor->proximo" no lugar de NULL
        x->anterior = menor->anterior;
        //------
      }
    }
